...
public interface VisitorVehicleRepository extends JpaRepository<VisitorVehicle, Long> {
    ...
    // 단지와 방문 예정일, 상태 기준 방문차량 건수를 집계한다.
    long countByComplexIdAndVisitDateAndStatusAndIsDeletedFalse(Long complexId, LocalDate visitDate, VisitorVehicleStatus status);

    // 단지와 방문 예정일 이후, 상태 기준 방문차량 건수를 집계한다.
    long countByComplexIdAndVisitDateGreaterThanEqualAndStatusAndIsDeletedFalse(Long complexId, LocalDate visitDate, VisitorVehicleStatus status);

    // 단지와 방문 예정일 기간 기준 방문차량 건수를 집계한다.
    long countByComplexIdAndVisitDateBetweenAndIsDeletedFalse(Long complexId, LocalDate fromDate, LocalDate toDate);
    ...
    @Query("""
            ...
            """)
    Page<VisitorVehicle> findHouseholdVisitorVehicles(
    ...
    // 내 세대 방문차량 목록 페이지 조회, 상태와 기간 동적 필터
    @Query("""
            SELECT v FROM VisitorVehicle v
            WHERE v.householdId = :householdId
              AND v.complexId = :complexId
            ...
            ORDER BY v.createdAt DESC
            """)
    Page<VisitorVehicle> findHouseholdVisitorVehiclesByStatus(
            @Param("householdId") Long householdId,
            @Param("complexId") Long complexId,
    ...
    @Query("""
            ...
            WHERE v.complexId = :complexId
              ...
              AND (:keyword IS NULL OR :keyword = ''
                   ...
                   OR v.visitPurpose LIKE CONCAT('%', :keyword, '%')
