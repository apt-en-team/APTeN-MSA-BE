...
// 매월 1일 0시에 모든 단지 직전 월 방문차량 비용을 산정하는 스케줄러이다.
@Slf4j
@Component
@RequiredArgsConstructor
@ConditionalOnProperty(name = "apten.scheduler.monthly-visitor-fee.enabled", havingValue = "true")
public class MonthlyVisitorFeeScheduler {
    ...
    // 직전 월 방문차량 비용 산정을 단지 단위 격리 실행
    @Scheduled(cron = "${apten.scheduler.monthly-visitor-fee.cron}")
    @SchedulerLock(name = "monthly-visitor-fee", lockAtMostFor = "30m", lockAtLeastFor = "1m")
    public void runMonthlyCalculation() {
        ...
        for (ComplexCache complex : complexes) {
            try {
                parkingFeeCalculationService.calculateVisitorFees(
                        VisitorFeeCalculateReq.builder()
                ...
                failureCount++;
                log.warn("[monthly-visitor-fee] calculation failed for complexId={}, reason={}",
                        complex.getId(), ex.getMessage());
            }
        }

        log.info("[monthly-visitor-fee] period={}-{}, success={}, failure={}",
